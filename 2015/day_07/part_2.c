...
int main() {
	...
	data_t memory_b[statements_len];

	compute_solution(statements, statements_len, memory, &mem_len);

	char *b_str = "b";
	for (size_t i = 0; i < mem_len; i++) {
		data_t *d = memory + i;
		if (strncmp("a", d->key->ptr, d->key->len) != 0 || d->key->len != 1) continue;
		...
		memory_b[0].key = &s;
		memory_b[0].value = d->value;
		break;
	}

	mem_len = 1;
	compute_solution(statements, statements_len, memory_b, &mem_len);
	for (size_t i = 0; i < mem_len; i++) {
		data_t *d = memory_b + i;
