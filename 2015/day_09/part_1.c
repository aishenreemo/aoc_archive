...
void solve(uint start, uint length, uint *shortest);
...
int main() {
	// a set of strings and a dictionary<str><uint> declaration
	...
	// debugging purposes
	// for (int i = 0; i < PLACES_CAPACITY; i++) {
	// 	if (places[i][0] == '\0') break;
	// 	printf("%s\n", places[i]);
	// }

	// for (int i = 0; i < DISTANCES_CAPACITY; i++) {
	// 	if (distances[i].source_to_dest[0] == '\0') break;
	// 	printf("%i %s: %i\n", i, distances[i].source_to_dest, distances[i].distance);
	// }
...
void solve(uint start, uint length, uint *shortest) {
	...
	if (start == length) {
		...
		*shortest = (*shortest < tmp_distance) ? *shortest : tmp_distance;
		return;
	}

	for (int i = start; i <= length; i++) {
		swap((places + start), (places + i));
		solve(start + 1, length, shortest);
		swap((places + start), (places + i));
	}
}
