...
int main() {
	...
	// include myself on the list
	for (uint i = 0; i < person_vec.length; i++) {
		line_t *l1 = malloc(sizeof(line_t));
		line_t *l2 = malloc(sizeof(line_t));
		...
		char *p1 = (char *) vector_get_f(&person_vec, i);
		char *p2 = "Myself";
		strcpy(l1->from, p1);
		strcpy(l1->to, p2);
		strcpy(l2->from, p2);
		...
		vector_push_f(&line_vec, l1);
		vector_push_f(&line_vec, l2);
	}

	char *myself = calloc(sizeof(char), 20);
	strcpy(myself, "Myself");
	if (!insert_person(&person_vec, myself)) free(myself);
